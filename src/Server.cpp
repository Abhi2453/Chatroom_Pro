...
void handle_client(int client_socket, int id) {
    ...
    while (1) {
        ...
        if (str[0] == '@') {
            ...
            size_t pos = string(str).find(':');
            if (pos != string::npos) {
                recipient_name = string(str).substr(1, pos - 1); // Extract recipient's name
                message_content = string(str).substr(pos + 1);   // Extract message content
            }

            // Find the recipient's socket based on their name
            ...
            if (recipient_socket != -1) {
                string private_message = string(name) + " (private): " + message_content;
                send(recipient_socket, private_message.c_str(), private_message.size(), 0);
            } else {
                // Notify the sender that the recipient is not available
                string unavailable_message = "Recipient '" + recipient_name + "' is not available.";
                send(client_socket, unavailable_message.c_str(), unavailable_message.size(), 0);
            }
        } else {
